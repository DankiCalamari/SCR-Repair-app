...
export default function NewQuoteModal({ open, onClose }: Props) {
  ...
  const { data: repairsData } = useQuery({
    queryKey: ["modal-repairs", repairSearch],
  ...
  return (
    <Modal open={open} onClose={resetAndClose} title="New Quote" maxWidth="max-w-xl">
      <div className="mb-6 flex items-center gap-2 text-sm">
        <span className={`font-medium ${step === "repair" ? "text-accent-500" : "text-surface-400"}`}>Repair</span>
        <ChevronRight className="h-3 w-3 text-surface-600" />
        <span className={`font-medium ${step === "details" ? "text-accent-500" : "text-surface-400"}`}>Details</span>
      </div>
          ...
          <div className="max-h-64 space-y-1 overflow-y-auto">
            {repairs.map((r) => (
              ...
              >
                ...
                <div>
                  <p className="text-sm font-medium text-surface-100">{r.ticket_number}</p>
                  <p className="text-xs text-surface-400">{r.issue_description.slice(0, 60)}{r.issue_description.length > 60 ? "..." : ""}</p>
                </div>
              </button>
            ))}
            {repairs.length === 0 && (
      ...
      {step === "details" && selectedRepair && (
        <div>
          <p className="mb-4 text-sm text-surface-400">
            Repair: <span className="font-medium text-surface-100">{selectedRepair.ticket_number}</span>
          </p>
          <div className="space-y-4">
            ...
            <div>
                ...
                {items.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                    ...
                    <input
                      ...
                      className="w-16 rounded-lg border border-surface-600 bg-surface-700 px-2 py-2 text-sm text-surface-100 focus:border-accent-500 focus:outline-none"
                    />
                    <input
                      type="number"
                      min="0"
              ...
              <button
                onClick={addItem}
                className="mt-2 flex items-center gap-1 text-xs text-accent-500 hover:text-accent-400"
              >
            ...
            <div className="rounded-lg border border-surface-600 bg-surface-700 p-4">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-surface-100">${subtotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-surface-400">GST (10%)</span>
                <span className="text-surface-100">${gst.toFixed(2)}</span>
              </div>
              <div className="mt-2 flex justify-between border-t border-surface-600 pt-2">
                <span className="font-medium text-surface-300">Total</span>
                <span className="font-semibold text-accent-500">${total.toFixed(2)}</span>
              </div>
            </div>
          </div>
