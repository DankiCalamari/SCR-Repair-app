...
export default function AdminCommunicationsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-8">
        <h1 className="font-heading text-3xl font-bold text-surface-100">Communications</h1>
      ...
      <div className="mb-6 flex gap-1 rounded-lg border border-surface-800 bg-surface-900 p-1 w-fit">
        <button
          ...
          className={cn(
            ...
            tab === "inbox" ? "bg-accent-500 text-surface-950" : "text-surface-400 hover:text-surface-100"
          )}
        >
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition",
            tab === "conversations" ? "bg-accent-500 text-surface-950" : "text-surface-400 hover:text-surface-100"
          )}
        >
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition",
            tab === "leads" ? "bg-accent-500 text-surface-950" : "text-surface-400 hover:text-surface-100"
          )}
        >
        ...
        <div className="rounded-lg border border-surface-800 bg-surface-900">
          <div className="border-b border-surface-800 px-5 py-4">
            <div className="flex items-center justify-between">
              <h2 className="font-heading text-lg font-semibold text-surface-100">Unassigned Messages</h2>
              <button
          ...
          <div className="divide-y divide-surface-800">
            {isLoading ? (
              ...
              <div className="px-5 py-24 text-center">
                ...
                <p className="mt-4 text-surface-400">No unassigned communications</p>
                <p className="mt-1 text-xs text-surface-500">All messages have been linked to customers</p>
              </div>
            ) : (
                ...
                <div key={`${item.type}-${item.id}`} className="px-5 py-4">
                  <div className="flex items-start gap-4">
                    <div className={cn(
                      ...
                      item.type === "email" ? "bg-accent-500/10 text-accent-500" :
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-surface-100">{item.from}</span>
                          <span className="rounded bg-surface-700 px-1.5 py-0.5 text-[10px] font-medium uppercase text-surface-300">
                      ...
                      {item.subject && (
                        <p className="mt-0.5 text-sm font-medium text-surface-200">{item.subject}</p>
                      )}
                      <p className="mt-1 text-sm text-surface-300 line-clamp-2 whitespace-pre-line">{item.body}</p>
                          ...
                          {item.type === "lead" ? (
                            <>
                              ...
                              <button
                                ...
                                className="flex items-center gap-1.5 rounded-lg border border-surface-700 px-3 py-1.5 text-xs font-medium text-surface-300 hover:border-accent-500 hover:text-accent-500 transition"
                              >
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-surface-800 px-5 py-4">
              <p className="text-xs text-surface-400">Page {page + 1} of {totalPages}</p>
      ...
      {tab === "conversations" && !activeConversation && (
        <div className="rounded-lg border border-surface-800 bg-surface-900">
          <div className="border-b border-surface-800 px-5 py-4">
            <h2 className="font-heading text-lg font-semibold text-surface-100">Conversations</h2>
            <p className="mt-1 text-sm text-surface-400">All email and SMS threads grouped by contact</p>
          </div>
          ...
          <div className="divide-y divide-surface-800">
            {isLoadingConversations ? (
              ...
              <div className="px-5 py-24 text-center">
                ...
                <p className="mt-4 text-surface-400">No conversations yet</p>
              </div>
            ) : (
              conversationsData?.data.map((conv: Conversation) => (
                <button
                  ...
                  className="w-full text-left px-5 py-4 hover:bg-surface-800 transition"
                >
                  <div className="flex items-start gap-4">
                    <div className={cn(
                      "mt-0.5 flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold",
                      conv.last_message_direction === "inbound" ? "bg-accent-500/10 text-accent-500" : "bg-surface-700 text-surface-300"
                    )}>
                      {conv.contact_name.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-surface-100">{conv.contact_name}</span>
                        <span className="text-[10px] text-surface-500">{formatDateTime(conv.last_message_at)}</span>
                      </div>
                      ...
                      <p className="mt-1 text-sm text-surface-400 line-clamp-1">{conv.last_message_body}</p>
                    </div>
                    {conv.unread_count > 0 && (
                      <span className="rounded-full bg-accent-500 px-2 py-0.5 text-[10px] font-bold text-surface-950">
                        {conv.unread_count}
                      </span>
                    )}
                  </div>
          ...
          {convTotalPages > 1 && (
            <div className="flex items-center justify-between border-t border-surface-800 px-5 py-4">
              <p className="text-xs text-surface-400">Page {page + 1} of {convTotalPages}</p>
              <div className="flex gap-2">
                <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0}
                  className="flex items-center gap-1 rounded border border-surface-700 px-3 py-1 text-xs text-surface-100 hover:bg-surface-800 disabled:opacity-50">
                  <ChevronLeft className="h-3 w-3" /> Previous
          ...
          <div className="border-b border-surface-800 px-5 py-4">
            <div className="flex items-center gap-3">
              <button
                ...
                className="rounded-lg border border-surface-700 p-1.5 text-surface-400 hover:bg-surface-800 hover:text-surface-100"
              >
                <ArrowLeft className="h-4 w-4" />
              </button>
              <div>
                <h2 className="font-heading text-lg font-semibold text-surface-100">{activeConversation.contact_name}</h2>
              ...
              <div key={msg.id} className={cn("flex", msg.direction === "outbound" ? "justify-end" : "justify-start")}>
                <div className={cn(
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] text-surface-600">{formatDateTime(msg.created_at)}</span>
                  </div>
                  {msg.subject && (
                    <p className="text-sm font-medium text-surface-200 mb-1">{msg.subject}</p>
                  )}
          ...
          <div className="border-t border-surface-800 px-5 py-4">
            <div className="mb-3 flex items-center gap-2">
              <button
                ...
                className={cn(
                  ...
                  replyType === "email" ? "bg-accent-500/10 text-accent-500 border border-accent-500/30" : "border border-surface-700 text-surface-400 hover:text-surface-100"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition",
                  replyType === "sms" ? "bg-purple-500/10 text-purple-400 border border-purple-500/30" : "border border-surface-700 text-surface-400 hover:text-surface-100"
                )}
              >
            ...
            {replyType === "email" && (
              <input
                ...
                className="mb-2 w-full rounded-lg border border-surface-700 bg-surface-800 px-4 py-2 text-sm text-surface-100 placeholder-surface-500 focus:border-accent-500 focus:outline-none"
              />
            )}
            ...
            <div className="flex gap-2">
              <textarea
                ...
                rows={3}
                className="flex-1 rounded-lg border border-surface-700 bg-surface-800 px-4 py-2.5 text-sm text-surface-100 placeholder-surface-500 focus:border-accent-500 focus:outline-none resize-none"
              />
              <button
                onClick={() => {
                ...
                className="flex items-center gap-2 self-end rounded-lg bg-accent-500 px-4 py-2.5 text-sm font-semibold text-surface-950 hover:bg-accent-400 disabled:opacity-50"
              >
            ...
            {replyType === "email" && emailTemplates && emailTemplates.length > 0 && (
              <div className="mt-3">
                <p className="text-xs text-surface-500 mb-1.5">Quick templates:</p>
                <div className="flex flex-wrap gap-1.5">
                  {emailTemplates.map((t: EmailTemplate) => (
                    <button
                      key={t.id}
                      onClick={() => { setReplySubject(t.subject); setReplyBody(t.body); }}
                      className="rounded border border-surface-700 px-2 py-1 text-[10px] text-surface-400 hover:border-accent-500 hover:text-accent-400 transition"
                    >
                      {t.name}
                    </button>
                  ))}
      ...
      {tab === "leads" && (
        <div className="rounded-lg border border-surface-800 bg-surface-900">
          <div className="border-b border-surface-800 px-5 py-4">
            <h2 className="font-heading text-lg font-semibold text-surface-100">Contact Form Submissions</h2>
            <p className="mt-1 text-sm text-surface-400">Website contact form enquiries that need follow-up</p>
          </div>
          ...
          <div className="divide-y divide-surface-800">
            {isLoading ? (
              ...
              <div className="px-5 py-24 text-center">
                ...
                <p className="mt-4 text-surface-400">No contact form submissions</p>
                <p className="mt-1 text-xs text-surface-500">Submissions from the website will appear here</p>
              </div>
            ) : (
              data?.data?.filter((d: UnassignedCommunication) => d.type === "lead")?.map((lead: UnassignedCommunication) => (
                <div key={lead.id} className="px-5 py-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-surface-100">{lead.from}</span>
                        <span className="text-[10px] text-surface-500 uppercase tracking-wider">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-surface-400">
                        ...
                        <span className="rounded bg-surface-700 px-1.5 py-0.5 text-[10px] uppercase">
                          Prefers: {lead.preferred_contact_method}
                        </span>
                      </div>
                      <p className="mt-2 text-sm text-surface-300">{lead.subject?.replace("Enquiry: ", "")}</p>

                      {expandedLead === lead.id ? (
                        <div className="mt-3 space-y-3">
                          <div className="rounded-lg bg-surface-800 p-3">
                            <p className="text-xs text-surface-500 mb-1">Issue Description</p>
                            <p className="text-sm text-surface-200 whitespace-pre-wrap">{lead.body}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                            ...
                            <button
                              ...
                              disabled={updateLeadMutation.isPending}
                              className="flex items-center gap-1.5 rounded-lg border border-surface-700 px-4 py-2 text-sm font-medium text-surface-300 hover:border-accent-500 hover:text-accent-500 transition"
                            >
                            ...
                            <button
                              ...
                              disabled={updateLeadMutation.isPending}
                              className="flex items-center gap-1.5 rounded-lg border border-surface-700 px-4 py-2 text-sm font-medium text-surface-400 hover:border-red-500/50 hover:text-red-400 transition"
                            >
                              Close
                            </button>
                          </div>
                        ...
                        <button
                          ...
                          className="mt-2 flex items-center gap-1.5 text-xs font-medium text-accent-400 hover:text-accent-300"
                        >
                          View details &rarr;
                        </button>
                      )}
