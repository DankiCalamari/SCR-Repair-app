  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="font-heading text-3xl font-bold text-warm-50">Email</h1>
          <p className="mt-1 text-warm-400">Send and track email messages</p>
        </div>
        <button
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-lg border border-warm-800 bg-warm-900 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-warm-400">SMTP</p>
        ...
        <div className="rounded-lg border border-warm-800 bg-warm-900 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-warm-400">IMAP</p>
        ...
        <div className="flex gap-2 rounded-lg border border-warm-800 bg-warm-900 p-4">
          <button
            onClick={() => testMutation.mutate()}
            disabled={testMutation.isPending}
            className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-warm-700 px-3 py-1.5 text-xs text-warm-50 hover:bg-warm-800"
          >
            Test Connections
          </button>
          <button
      ...
      {/* Test Results */}
      {testResults && (
        <div className="mb-6 rounded-lg border border-warm-800 bg-warm-900 p-4">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-warm-50">Test Results</h3>
            <button onClick={() => setTestResults(null)} className="text-warm-400 hover:text-warm-50">
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className={cn(
              ...
              <p className="mt-1 text-xs text-warm-300">{testResults.smtp.message}</p>
              {testResults.smtp.host && (
                <p className="mt-0.5 text-xs text-warm-500">{testResults.smtp.host}:{testResults.smtp.port}</p>
              )}
            </div>
            <div className={cn(
              "rounded-lg border p-3",
              ...
              <p className="mt-1 text-xs text-warm-300">{testResults.imap.message}</p>
              {testResults.imap.host && (
                <p className="mt-0.5 text-xs text-warm-500">{testResults.imap.host}:{testResults.imap.port}</p>
              )}
            </div>
          </div>
        </div>
      ...
      {tab === "messages" && (
        <>
          ...
          <div className="mb-6 rounded-lg border border-warm-800 bg-warm-900 p-5">
            <h2 className="mb-4 font-heading text-lg font-semibold text-warm-50">Send Email</h2>
            <div className="space-y-3">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg bg-copper-500 px-4 py-2.5 font-semibold text-warm-950 hover:bg-copper-600 disabled:opacity-50"
              >
          ...
          <div className="rounded-lg border border-warm-800 bg-warm-900">
            <div className="border-b border-warm-800 px-5 py-4">
              <h2 className="font-heading text-lg font-semibold text-warm-50">Email History</h2>
            </div>
            <div className="divide-y divide-warm-800">
              {isLoading ? (
                ...
                <div className="px-5 py-12 text-center text-warm-400">No emails yet</div>
              ) : (
                data?.data?.map((email: EmailMessage) => (
                  <div key={email.id} className="flex items-start gap-4 px-5 py-4">
                    <div className={cn(
                      "mt-0.5 rounded-full p-2",
                      email.direction === "outbound" ? "bg-copper-500/10" : "bg-warm-700"
                    )}>
                      <Mail className={cn(
                        "h-4 w-4",
                        email.direction === "outbound" ? "text-copper-500" : "text-warm-300"
                      )} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-warm-50 truncate">{email.subject}</span>
                        <span className={cn(
                      ...
                      <p className="mt-1 text-sm text-warm-300 truncate">{email.body}</p>
                      <p className="mt-1 text-xs text-warm-500">{formatDateTime(email.created_at)}</p>
                    </div>
                  </div>
                ))
              )}
      ...
      {tab === "templates" && (
        <div className="rounded-lg border border-warm-800 bg-warm-900">
          <div className="border-b border-warm-800 px-5 py-4">
            <h2 className="font-heading text-lg font-semibold text-warm-50">Email Templates</h2>
              ...
              templates.map((t) => (
                <div key={t.id} className="flex items-center justify-between gap-4 px-5 py-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-warm-400">Subject: {t.subject}</p>
                    <p className="mt-1 text-sm text-warm-300 line-clamp-2">{t.body}</p>
      ...
      {previewTemplate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setPreviewTemplate(null)}>
          <div className="w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-lg border border-warm-800 bg-warm-900 p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-3">
              <p className="text-xs font-medium text-warm-400 mb-1">Subject</p>
              <p className="text-sm text-warm-50 font-mono bg-warm-950 rounded px-3 py-2">{previewTemplate.subject}</p>
            </div>
            <div className="mb-4">
              <p className="text-xs font-medium text-warm-400 mb-1">Body</p>
              <div className="rounded-lg bg-warm-950 p-4">
                <p className="text-sm text-warm-200 whitespace-pre-wrap">{previewTemplate.body}</p>
              </div>
            </div>
