  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="font-heading text-3xl font-bold text-surface-100">Leads</h1>
            <p className="mt-1 text-surface-400">{data?.total ?? 0} total leads</p>
          </div>
          <button className="flex items-center gap-2 rounded-lg bg-accent-500 px-4 py-2.5 font-semibold text-surface-950 hover:bg-accent-400">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-surface-800">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-5 py-12 text-center text-surface-400">No leads found</td>
                </tr>
              ) : (
                data?.data?.map((lead: Lead) => (
                  <tr key={lead.id} className="transition hover:bg-surface-800">
                    <td className="px-5 py-4">
                      <span className="font-medium text-surface-100">{lead.name}</span>
                    </td>
                    <td className="px-5 py-4">
                      <p className="text-sm text-surface-300">{lead.phone}</p>
                    ...
                    <td className="px-5 py-4 text-sm text-surface-400">{lead.source || "—"}</td>
                    <td className="px-5 py-4 text-sm text-surface-400">{formatDate(lead.created_at)}</td>
                    <td className="px-5 py-4">
